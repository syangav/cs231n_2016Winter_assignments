...
def affine_backward(dout, cache):
  ...
  d_1tilld_k = x.shape[1:]
  temp = np.insert(d_1tilld_k, 0, x.shape[0])
  dx = np.reshape(dout.dot(w.T), temp)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x, sample_mean, sample_variance, x_normalized, gamma, beta, eps)
    #############################################################################
...
def batchnorm_backward(dout, cache):
  ...
  x, sample_mean, sample_variance, x_normalized, gamma, beta, eps = cache
  N = x.shape[0]
  dbeta = 1 * np.sum(dout, axis = 0)
  dgamma = np.sum(x_normalized * dout, axis = 0)
  dx_normalized = dout * gamma 
  k = np.sqrt(sample_variance)
  dsample_variance = np.sum(-1.0 * (1.0 / (k + eps)**2) * (x - sample_mean) * 1/2 * (1.0 / k) * dx_normalized, axis = 0)
  dsample_mean = np.sum(-1 * (1 / (eps + k)) * dx_normalized, axis = 0)
  dx = dx_normalized / (k + eps) + (1.0 / N) * dsample_mean + (1.0 / N) * 2.0 * (x - sample_mean) * dsample_variance
  #############################################################################
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx_normalized = dout * gamma 
  k = np.sqrt(sample_variance)
  dsample_variance = np.sum(-1.0 / 2 * (1.0 / k) * (1.0 / (k + eps)) * x_normalized * dx_normalized, axis = 0)
  dsample_mean = np.sum(-1.0 / (eps + k) * dx_normalized, axis = 0)
  dx = dx_normalized / (k + eps) + (1.0 / N) * dsample_mean + (1.0 / N) * 2.0 * (x - sample_mean) * dsample_variance
  #############################################################################
  # TODO: Implement the backward pass for batch normalization. Store the      #
  # results in the dx, dgamma, and dbeta variables.                           #
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    # here p means the fraction of the DROPPED neurons 
    N, D = x.shape
    mask = (np.random.rand(N, D) < (1.0 - p)) / (1.0 - p)
    out = x * mask
    ###########################################################################
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
    for c in range(C):
        for h in range(H_out):
            for w in range(W_out):
                local_max = np.max(x[n, c, h * stride: h * stride + pool_height, w * stride: w * stride + pool_width])
                for i in range(h * stride, h * stride + pool_height):
                    for j in range(w * stride, w * stride + pool_width):
                        if x[n, c, i, j] == local_max:
                            dx[n, c, i, j] += dout[n, c, h, w]
