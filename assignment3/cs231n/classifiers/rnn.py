...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'rnn':
        rnn_out, rnn_cache = rnn_forward(embed_out, initial_h, Wx, Wh, b)
        # (4)
        affine_out, affine_cache = temporal_affine_forward(rnn_out, W_vocab, b_vocab)
    else:
        lstm_out, lstm_cache = lstm_forward(embed_out, initial_h, Wx, Wh, b)
    ...
    daffine, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, affine_cache)
    if self.cell_type == 'rnn':
        drnn, dinitial_h, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(daffine, rnn_cache)
        grads['W_embed'] = word_embedding_backward(drnn, embed_cache)
    else:
        dlstm, dinitial_h, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(daffine, lstm_cache)
        grads['W_embed'] = word_embedding_backward(dlstm, embed_cache)
    grads['W_proj'] = features.T.dot(dinitial_h)
    grads['b_proj'] = np.sum(dinitial_h, axis = 0)
    ############################################################################
